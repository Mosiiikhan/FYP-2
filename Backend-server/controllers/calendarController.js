...
// 2. 🟢 MAIN FIX: Semester-wise Events Fetching (AD Logic Synced)
exports.getAllEvents = async (req, res) => {
    try {
        ...
        if (isStudent) {
            // Hum yahan Notifications table ko join kar rahay hain kyunke AD ne notification pehle hi bhej di hai
            // Is se semester wala crash khatam ho jayega
            query += ` UNION ALL
        ...
        // --- PART 4: Exams (Including AD in Roles) ---
        if (normalizedRole === 'admin' || normalizedRole === 'datacell' || normalizedRole === 'chairperson' || normalizedRole === 'assistant director') {
            query += ` UNION ALL
                      SELECT CAST(e.exam_id AS VARCHAR) as id, 'Exam: ' + c.course_name as title, 'exam' as type, 
                             ...
                             ISNULL(e.color_code, '#dc3545') as color_code, CAST(e.start_time AS VARCHAR) as start_time, e.room_no
                      FROM Exam_Schedule e JOIN Courses c ON e.course_id = c.course_id`;
        } else if (isStudent) {
            query += ` UNION ALL
                      SELECT CAST(e.exam_id AS VARCHAR) as id, 'Exam: ' + c.course_name as title, 'exam' as type, 
                             CONVERT(VARCHAR, e.exam_date, 23) as start_date, CONVERT(VARCHAR, e.exam_date, 23) as end_date, 
                             ISNULL(e.color_code, '#dc3545') as color_code, CAST(e.start_time AS VARCHAR) as start_time, e.room_no
                      FROM Exam_Schedule e JOIN Courses c ON e.course_id = c.course_id
                      JOIN Enrollments en ON e.course_id = en.course_id WHERE en.student_id = @studentId`;
        }
